...
export const google = async (req, res, next) => {
    try{
        ...
        if (user) {
            ...
            const newUser = new User({ username: req.body.name.split(" ").join("").toLowerCase() + Math.floor(Math.random * 10000).toString(), email: req.body.email, user_password: hashedPassword});
            await newUser.save()
